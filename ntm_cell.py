...
class NTMCell():  #this is the class for the NTM cell . Same as LSTM cell but with external memory as in the paper 
    ...
    def __call__(self, x, prev_state):
        ...
        with tf.variable_scope('controller', reuse=self.reuse):
            controller_output, controller_state = self.controller(controller_input, prev_controller_state) #send through the lstm
        ...
        num_parameters_per_head = self.memory_vector_dim + 1 + 1 + (self.shift_range * 2 + 1) + 1 #here kt means 20 dim thing 
        ...
        for i, head_parameter in enumerate(head_parameter_list): #read right head weights 
            ...
            k = tf.tanh(head_parameter[:, 0:self.memory_vector_dim]) #getting the K values  inoder to calculate the content based similarity 
            beta = tf.sigmoid(head_parameter[:, self.memory_vector_dim]) * 10    # do not use exp, it will explode!
            g = tf.sigmoid(head_parameter[:, self.memory_vector_dim + 1])  #this is the interpolation gate 
    ...
    def addressing(self, k, beta, g, s, gamma, prev_M, prev_w):
        ...
        w_g = g * w_c + (1 - g) * prev_w               # eq (7) element wise multiplication 
        ...
        with tf.variable_scope('init', reuse=self.reuse):
            state = {            #state is a tupple 
                ...
                'controller_state': expand(tf.tanh(tf.get_variable('init_state', self.rnn_size,
                                            initializer=tf.random_normal_initializer(mean=0.0, stddev=0.5))),
                                  dim=0, N=batch_size),
                'read_vector_list': [expand(tf.nn.softmax(tf.get_variable('init_r_%d' % i, [self.memory_vector_dim], #this is for the read vector weights
                                            initializer=tf.random_normal_initializer(mean=0.0, stddev=0.5))),
                                  dim=0, N=batch_size)
                           for i in range(self.read_head_num)],
                'w_list': [expand(tf.nn.softmax(tf.get_variable('init_w_%d' % i, [self.memory_size],   #this is the weight vec list 
                                            initializer=tf.random_normal_initializer(mean=0.0, stddev=0.5))),
